...
// IR Sensor variables
//if (scannerIr == 1) {
  SharpIR scannerIrSensor[] = {
    SharpIR(scannerIrPin[0], irModel),
    SharpIR(scannerIrPin[1], irModel)
  };
//}

//if (frontUs == 1) {
  SharpIR  frontIrSensor = SharpIR(frontIrPin, irModel);
//}
...
void setup() {
  ...
  for (int i = 0; i < 180; i = i + 1) {
    ...
    scannerIrMeasure[i] = maxIrDistance;
  }
  // Initiate front measure array
  frontUsMeasure[0] = currentTime;
  frontUsMeasure[1] = -1;
  frontUsMeasure[2] = -1;
  frontIrMeasure[0] = currentTime;
  frontIrMeasure[1] = -1;
  frontIrMeasure[2] = -1;  
}
...
String collectData(int angle) {
  ...
  irDistance = frontIrSensor.distance();
  currentTime = millis();
  ...
  // ir
  if (irDistance >= maxIrDistance) {
    irDistance = frontIrMeasure[1];
    irSpeed = frontIrMeasure[2];
  } else {
    irSpeed = getSpeed(frontIrMeasure[0],frontIrMeasure[1],irDistance,maxIrDistance);
  }
  frontUsMeasure[0] = currentTime;
  ...
  frontIrMeasure[0] = currentTime;
  frontIrMeasure[1] = irDistance;
  frontIrMeasure[1] = irSpeed;
  ...
  for (int i = 0; i < 2; i = i + 1) {
      ...
      irDistance = scannerIrSensor[i].distance();
      currentTime = millis();
      // us
      if (usDistance >= maxUsDistance) {
      ...
      // ir
      if (irDistance >= maxIrDistance) {
        irDistance = scannerIrMeasure[angle  + scannerServoAngleMax * i];
        irSpeed = -1;
      } else {
        irSpeed = getSpeed(scannerTime[angle],scannerIrMeasure[angle  + scannerServoAngleMax * i],irDistance,maxIrDistance);
      }
      ...
      Serial.println(s);
      scannerIrMeasure[angle  + scannerServoAngleMax * i] = irDistance;
      s=s + "4:" + String(angle  + scannerServoAngleMax * i) + ":" + String(irDistance) + ":" + String(irSpeed);
      Serial.println(s);
...
int getIrDistance(int irNr) {
  int irvalues[25];
  int result = scannerIrSensor[irNr].distance();
