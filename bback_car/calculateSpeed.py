...
# CalculateSpeed node is calculating speed by driving forward againt a wall
# with different motor speed
# based on the result we build a speed matrix
#
# A table containing motorpower and speed separated by :
# If we do not have this file the first goal is to do the calculation to be able to estimate how far we have moved
#
class CalculateSpeed(Node):

    def __init__(self):
        super().__init__('calculatespeed')
        # #topic publisher motorpower
        ...
        self.subscription_motorstatus = self.create_subscription(
            Int16MultiArray,
            'motorstatus',
            self.motorstatus_listener_callback, 1)
